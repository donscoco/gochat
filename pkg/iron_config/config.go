...
func convertNodes(tree map[string]*node, path string, obj interface{}) {
	...
	case reflect.Map:
		for _, key := range v.MapKeys() {
			convertNodes(tree, path+SEP+key.String(), v.MapIndex(key).Interface())
		}
	case reflect.Slice:
		tree[path+SEP+"length"] = &node{i: v.Len()}
		for i := 0; i < v.Len(); i++ {
			convertNodes(tree, path+SEP+strconv.Itoa(i), v.Index(i).Interface())
		}
	case reflect.String:
